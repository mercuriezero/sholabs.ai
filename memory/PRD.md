...
## Rev 29 (2026-06 fork) — Full multi-service portal + paywall gating + SDR removal + pricing mobile fix
- Portal expanded to a full app (new sidebar): Dashboard (/portal, overview cards + stats via /api/portal/overview), LLM Citation (/portal/llm = scan/opportunities/pages), AI Video (/portal/video), Voice AI (/portal/voice), Leads (/portal/leads), Affiliate (/portal/affiliate), Fractional CXO Hours (/portal/hours).
- PAYWALL GATING (point 1): LLM scan + opportunity report stay FREE for any logged-in user; GENERATING/publishing pages is locked unless admin OR has >=1 successful payment. Backend: GET /api/portal/access {is_admin,has_paid,llm_unlocked}; POST /api/portal/generate-pages returns 402 if locked. Frontend: GeneratedPages shows pages-locked + unlock CTA; Opportunities generate handles 402 -> redirect to /fractional-cxo#cxo-packs.
- AI VIDEO studio (point 3, intake-only): form (project, type, ratio, language, duration, avatar, script, assets, notes) -> POST /api/portal/video-requests, status 'In production', shows 'Video generation in progress'. Team fulfils manually. GET lists requests. Owner email alert on submit.
- VOICE AI studio (point 6, intake-only): form (agent name, website, purpose, voice, language, greeting, knowledge, outcomes, phone, notes) -> POST /api/portal/voice-requests, status 'Building your agent'. GET lists. Owner alert.
- New collections: video_requests, voice_requests. Helper user_has_paid().
- AI SDR REMOVED ENTIRELY (point 2): Estimator (service + all sdr state/calc/case/deps), Portal.jsx (SDR tab + SdrPanel + PANELS.sdr + Send import), KeywordField (chip renamed to 'AI Agents', dup-key fixed), PromptBox, Leads copy, FractionalCxO demand-engine text, backend BOT/RESEARCH prompts, fractional-cxo metadata. Estimator math re-verified.
- PRICING mobile fix (point 5): the pricing page (/fractional-cxo) header top-right button (cxo-nav-cta) is now responsive — 'Book a call' <sm, 'Book a working session' >=sm; logo h-8 on mobile. No overflow at 390px.
- Legacy 'growth 360' Onboarding modal RETIRED (removed from providers) — the portal owns onboarding; it was blocking the pricing page for logged-in users.
...
## Rev 31 (2026-06 fork) — Leads demo mode + dashboard analytics + stack section removed
- Portal Leads (/portal/leads): added a "Demo data" toggle (leads-demo-toggle) CHECKED BY DEFAULT. Demo mode shows 13 sample leads (12 real + 1 spam) mapped to the service stack (LLM Citation, AI Video, Voice AI, Affiliate, Fractional CXO); status/notes edits update local state with a success toast. Unchecking shows the real portal_leads (empty state when none). Stat tiles compute over non-spam leads. Frontend-only demo data (PortalLeadsBoard.jsx).
- Portal Dashboard (/portal): added recharts analytics above the service cards — Live badge, 6 gradient KPI tiles (AI Citations / Share of Voice / Videos / Voice Minutes / Leads / Pipeline), "AI visibility & citations" area+line, "Leads & booked meetings" bar, "Leads by service" pie, "Goal progress" radial gauge (with a custom legend below to avoid overlap). All demo data, hardcoded in PortalDashboard.jsx. Added deps: recharts + react-is.
- Removed "The stack you'd otherwise need" section + StackCalculator from the marketing homepage (WhyUs.jsx). StackCalculator.jsx now orphaned (kept on disk, unused).
- Verified: testing agent iteration_7 — all 3 changes pass E2E; fixed the 2 reported cosmetic issues (gauge legend overlap/mismatch → custom legend; leads total counted spam → now non-spam only).
