...
import { IndianRupee, Loader2, Lock, X } from "lucide-react";
...
export default function PayButton({ label = "Pay now", testid = "pay-button" }) {
  const { user } = useAuth();
  ...
  const start = () => (user ? setAmountOpen(true) : setAuthOpen(true));
  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch(`${API}/payments/create-order`, {
        ...
        body: JSON.stringify({ amount_rupees: rupees }),
      });
      if (res.status === 401) {
        setAmountOpen(false);
        setAuthOpen(true);
      ...
      const rzp = new window.Razorpay({
        ...
        name: "High On AI",
        description: "Growth engine payment",
        ...
        handler: async (resp) => {
          ...
          if (v.ok) toast.success("Payment received — thank you! Our team will reach out within 24 hours.");
        ...
        {amountOpen && (
          ...
          >
            ...
            >
              ...
              <h2 className="font-display text-2xl font-semibold tracking-tight text-black">Choose an amount</h2>
                ...
                {PRESETS.map((p) => (
                  <button
                    ...
                    className={`rounded-2xl border px-3 py-3 text-sm font-semibold transition-colors ${
                      parseFloat(amount) === p ? "border-black bg-black text-white" : "border-neutral-200 text-neutral-600 hover:border-black/40"
                    }`}
                  >
              ...
              >
                ...
                {paying ? "Starting checkout…" : `Pay ${amount ? `₹${parseFloat(amount).toLocaleString("en-IN")}` : ""} securely`}
              </button>
